...
.deptname{ background:#f4f4f4; border-bottom:#d0d0d0 1px solid; color:#258abf;}
	...
	<div id="wrapper">
		...
		<div id="contents">
     	   ...
     	   <div class="boderWrap">
				...
				<form style="clear: both; width: 100%; padding:3px; vertical-align: middle;" id="search_frm" action="./PoliceStatisticsAction.sm" method="get"  >
				...
				<table id="sendResultList"  style="clear: both;" width="100%" border="0" cellpadding="0" cellspacing="0">
					...
					<c:forEach var="data"  items="${list}" >
						<tr>
							<td class='deptname' alt="${data.deptcode}"><a href="./DeptStatisticsAction.sm?pscode=${data.pscode}&deptcode=${data.deptcode}&type=${type}&date=${date}">
										${data.name}</a></td>
...
$(function(){
    ...
    allChartCreate = function(titleName){
 	...
 	$(".jan, .feb, .mar, .apr, .may, .jun, .jul, .aug, .sep, .oct, .nov, .dec ").click(function(){
 		var $this = $(this),
 			deptcode = $this.siblings().first().attr('alt'); 		
 		// 발송건수가 있는지 체크
 		if($.trim($this.text()) == '-'){
 			alert("발송건수가 없습니다.");
 			return;
